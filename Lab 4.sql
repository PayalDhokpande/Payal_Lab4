...
(select `order`.ord_amount, `order`.cus_id, customer.cus_gender, customer.cus_name from `order`
inner join
customer 
...
select `order`.*, t1.pro_name from `order`
inner join
(select  product.Pro_name, supplier_pricing.pricing_id from product 
inner join 
supplier_pricing
on
supplier_pricing.PRO_ID = product.PRO_ID) as t1
on
t1.PRICING_ID = `order`.PRICING_ID where cus_id = 2
;
...
inner join
(select Pro_id, min(supp_price)  as Min_Price from supplier_pricing group by pro_id) as t2
where t2.pro_id = product.pro_id)
 as t3
 where t3.cat_id= category.CAT_ID group by t3.cat_id;
/**we want least expensive product from each category hence grouped in last */


/*7)	Display the Id and Name of the Product ordered after “2021-10-05”.*/
select t1.pro_id, product.pro_name from product
inner join
(select `order`.ORD_ID, `order`.ord_date, supplier_pricing.pro_id from `order`
inner join
supplier_pricing 
on
`order`.PRICING_ID= supplier_pricing.PRICING_ID where `order`.ord_date>"2021-10-05") as t1
on
t1.pro_id = product.PRO_ID;
